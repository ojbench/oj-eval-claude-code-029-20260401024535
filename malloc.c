...
/* Test driver - reads commands from stdin */
int main(void)
{
    char cmd[10];
    void *ptrs[10000];
    ...
    while (scanf("%s", cmd) == 1) {
        if (strcmp(cmd, "malloc") == 0) {
            ...
            int idx;
            size_t size;
            if (scanf("%d %zu", &idx, &size) != 2) continue;
            if (idx >= 0 && idx < ptr_count) {
            ...
            size_t nmemb, size;
            if (scanf("%zu %zu", &nmemb, &size) != 2) continue;
